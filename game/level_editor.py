...
class LevelEditor(GameManager):
    ...
    def load_button(self):
        open_tile_img = resource.image('gridMenu.png')
        self.open_tile_btn = Button(
            open_tile_img,
            x=20,
            y=settings.SCREEN_OR_HEIGHT - 50,
            batch=self.batch,
            ...
            if key_left(k):
                ...
                for i in range(self.layer_repeat):
                    for j in range(self.layer_number):
                        ...
                        layer.x += self.scroll_speed * ((j+1) / self.layer_number)

            elif key_right(k):
                last_layer = self.layer_list[len(self.layer_list)-1]
                if last_layer.x + last_layer.width <= settings.SCREEN_OR_WIDTH:
                    break
                for i in range(self.layer_repeat):
                    for j in range(self.layer_number):
                        index = i * self.layer_number + j
